...
class OatppSwaggerConan(ConanFile):
    ...
    def configure(self):
        ...
        if self.settings.os == "Windows" and self.options.shared:
            raise ConanInvalidConfiguration(
                "oatpp-swagger can not be built as shared library on Windows")
    ...
    def source(self):
        ...
        os.rename(
            "oatpp-swagger-{0}".format(self.version), self._source_subfolder)
    ...
    def package_info(self):
        ...
        self.cpp_info.components["_oatpp-swagger"].includedirs = [
            os.path.join(
                "include", "oatpp-{}".format(self.version), "oatpp-swagger")
        ]
        self.cpp_info.components["_oatpp-swagger"].libdirs = [
            os.path.join("lib", "oatpp-{}".format(self.version))]
        ...
        # export env var
        res_path = os.path.join(self.package_folder, "include",
                                "oatpp-{}".format(self.version), "bin", "oatpp-swagger", "res")
        self.output.info(
            "Creating OATPP_SWAGGER_RES_PATH environment variable: {}".format(res_path))
